...
CONFIGURED=YES — `SMTP_HOST`, `SMTP_USERNAME` (`admin@botconnector.id`) are non-secret config defaults (host/username are not secrets); `SMTP_PASSWORD` was found as a **hardcoded, real-looking literal fallback** (not a placeholder), confirmed not overridden by any `.env` on the box — meaning it was the password actually in active use. **Fixed 2026-09-06 in BOTH the canonical copy (`apps/homepage-runtime`) AND the live source** (`/opt/botconnector-platform-starter-v0.3/homepage/app/support.py`, backed up + SHA256-recorded first) — both now require `SMTP_PASSWORD_FILE` or `SMTP_PASSWORD`, no hardcoded fallback, fail-closed for email sending only (ticket creation itself is unaffected). The running container was NOT restarted — it still serves from the old baked image, so production behavior hasn't changed yet. Full incident record: `docs/security/SMTP-CREDENTIAL-INCIDENT.md`.
SANDBOX_OR_LIVE=LIVE
...
FAILURE_HANDLING=CONFIRMED SAFE — send failures (including an auth failure from a missing/invalid credential) are caught, recorded in `support_email_outbox` as FAILED, and do not fail ticket creation
PUBLIC_READY=N/A — this is a notification side-channel for the support capability above, not a standalone public claim
STATUS=LIVE
NOTES=**Rotation is BLOCKED, not just pending**: the same mailbox (`admin@botconnector.id` / `smtp.hostinger.com`) is also used by `botconnector-backend-core`'s `worker`+`api` containers with a DIFFERENT password (hash-compared, confirmed different) — a system entirely outside this consolidation's scope. Rotating Hostinger's password for this account risks breaking that other system. See SMTP-CREDENTIAL-INCIDENT.md Phase 5 for the two things that must happen before rotation.
