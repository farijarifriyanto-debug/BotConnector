...
export async function api(pathname, { method = 'GET', body, timeoutMs = 30000, fetchImpl = fetch } = {}) {
  const key = getKey();
  if (!key) throw new Error('OLLAMA_API_KEY belum diset. Buat di https://ollama.com/settings/keys lalu: export OLLAMA_API_KEY=...');
  let res;
  try {
...
// GET /api/tags — verifikasi key + daftar model cloud yang bisa diakses.
export async function listModels(opts = {}) {
  const j = await api('/api/tags', opts);
