...
// ---------- eksekusi perintah (konten nyata) ----------
async function execParsed({ cmd, args }) {
  // kembalikan {role, text} agar renderer bisa mewarnai
  switch (cmd) {
    case 'hw': {
    ...
    case 'profiles': {
      ...
      return [{ role: 'agent', text: st.profiles.map((p) => `${p.active ? '●' : '○'} ${C.bold}${p.name}${C.reset}  ${C.dim}${p.baseUrl}${C.reset}  ${p.up ? `${C.green}UP ${p.status}${C.reset}` : `${C.red}DOWN ${p.error || '?'}${C.reset}`}`).join('\n') }];
    }
    case 'switch': {
      if (!args[0]) return [{ role: 'picker' }];
      const r = await switchProfile(args[0]);
      return [{ role: 'agent', text: `${C.green}Aktif: ${r.active}${C.reset} (health UP ${r.health.status})` }];
    }
    case 'add': {
    ...
    case 'mcp': {
      ...
      return [{ role: 'agent', text: (await listProbed()).map((r) => `${r.probe.toolOnPath ? `${C.green}+${C.reset}` : `${C.red}x${C.reset}`} ${C.bold}${r.id}${C.reset} [${r.license}] ${C.dim}${r.probe.note}${C.reset}`).join('\n') }];
    }
    case 'launch':
      return [{ role: 'agent', text: detectTools().map((t) => `${t.detected === null ? `${C.muted}-${C.reset}` : t.detected ? `${C.green}+${C.reset}` : `${C.red}x${C.reset}`} ${C.bold}${t.id}${C.reset} ${C.dim}:: ${t.config}${C.reset}`).join('\n') }];
    case 'help':
      return [{ role: 'agent', text: SLASH.map((s) => `${C.accent}${s.name}${C.reset} — ${s.desc}`).join('\n') }];
    case '':
      return [];
    default:
...
export async function runTui() {
  ...
  async function submit() {
    ...
    busy = true; render();
    try {
      const res = await execParsed({ cmd, args });
      for (const r of res) {
        if (r.role === 'picker') {
          const st = await liveStatus();
